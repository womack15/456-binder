...
int main()
{
	...
	for(int progCount = 0; 	progCount < NUM_BINARIES; ++progCount)
	{
		...
		FILE *ptrFile = fopen(tempFile, "wb");
        ...
        if(fwrite(codeArray[progCount], sizeof(char), programLengths[progCount], ptrFile) < programLengths[progCount])
        {
            perror("fwrite");
            exit(-1);
        }
